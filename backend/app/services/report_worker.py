...
def run_build_final_report(report_id: str):
    ...
    try:
        ...
        report_session_store.set_final_report(
            report_id,
            file_stream.read(),
        )
        logger.info("Successfully completed background build for report session %s", report_id)
    except Exception as exc:
        logger.exception("Failed background build for report session %s", report_id)
        report_session_store.set_final_report_error(report_id, str(exc))
...
def enqueue_build_final_report(report_id: str, background_tasks: BackgroundTasks):
    ...
    # Fallback to local thread/FastAPI background task
    background_tasks.add_task(run_build_final_report, report_id)
    logger.info("Enqueued report build task in local BackgroundTasks for report_id: %s", report_id)
